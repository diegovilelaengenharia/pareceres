...
def build_identificacao(doc, d):
    ...
    for i, (rot, val) in enumerate(linhas):
        ...
        set_spacing(p_rot, line=260, before=50, after=50)
        ...
        set_spacing(p_val, line=260, before=50, after=50)
        r_val = p_val.add_run(val)
        set_font(r_val, size=SZ_TABELA, bold=(i == 2))
...
def build_dados_carimbo(doc, d):
    ...
    set_spacing(p_tit, line=240, before=140, after=40)
...
def add_doc_item(doc, tipo, obs=None):
    ...
    add_run(p_tipo, tipo, bold=True, size=9)

    # Observação
    if obs:
        p_obs = cell.add_paragraph()
        p_obs.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        set_spacing(p_obs, line=260, before=0, after=40)
        r_obs = add_run(p_obs, obs, size=9, italic=True)
...
def build_conclusao_e_docs(doc, d):
    ...
    p_conc = add_para(doc, line=LINE_SPC, before=0,
                      after=PAR_AFTER, indent_cm=INDENT)
    add_run(p_conc, conclusao, size=SZ_CORPO)
