...
def processar_pdfs(pasta: str = PASTA_ENTRADA) -> list[str]:
    ...
    for pdf_path in pdfs:
        ...
        if metodo == "pdf_escaneado":
            print(f"  [!] PDF sem texto selecionável. Iniciando OCR local de alta fidelidade...")
            try:
                from extractors.ocr_engine import processar_ocr_local
                sucesso = processar_ocr_local(pdf_path, txt_path)
                if sucesso:
                    txts_gerados.append(txt_path)
                    continue
            except ImportError:
                print("  [!] Motor de OCR não encontrado.")
            
            # Fallback se o OCR falhar
