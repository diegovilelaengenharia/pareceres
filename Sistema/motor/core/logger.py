...
def get_logger():
    ...
    if not logger.handlers:
        ...
        # JSON Handler (Agent-First Observability) — log em Sistema/logs/
        _motor_dir   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # motor/
        _sistema_dir = os.path.dirname(_motor_dir)                                  # Sistema/
        _logs_dir    = os.path.join(os.path.dirname(_sistema_dir), "Sistema", "logs")
        os.makedirs(_logs_dir, exist_ok=True)
        log_file_json = os.path.join(_logs_dir, "motor.json.log")
