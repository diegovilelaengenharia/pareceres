...
from core.config import PROJECT_ROOT, PASTA_ENTRADA, PASTA_SAIDA, PASTA_MODELOS, PASTA_TREINO
...
PASTA_EXEMPLOS_IN  = os.path.join(PROJECT_ROOT, "Saida", "_Testes")
PASTA_EXEMPLOS_OUT = os.path.join(PROJECT_ROOT, "Saida", "_Testes")
...
def api_inspecionar(conteudo):
    ...
    try:
        ...
        from analyzers.inspetor_documental import inspecionar
...
def api_aprender(data):
    ...
    try:
        ...
        if tipo == "decisao_triagem":
            ...
            arquivo = os.path.join(PROJECT_ROOT, "Sistema", "inteligencia", "RETROALIMENTACAO_IA.md")
            with open(arquivo, "a", encoding="utf-8") as f:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/api/docs": self._json(api_listar_docs())
        elif path.startswith("/api/docs/"): self._json(api_ler_doc(urllib.parse.unquote(path[10:])))
    ...
    def do_POST(self):
        """Trata requisições POST para as APIs do painel."""
    ...
    def do_DELETE(self):
        """Trata requisições DELETE para remover arquivos JSON."""
        path = urllib.parse.urlparse(self.path).path
...
def main():
    ...
    print(f"  {DM}  │{R}  {YL}📋 Base:{R}     {DM}Sistema/base_conhecimento/      {R}  {DM}│{R}")
    print(f"  {DM}  └─────────────────────────────────────────────┘{R}")
    print()
