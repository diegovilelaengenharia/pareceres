...
def _texto_campo(dados: dict, *chaves) -> str:
    """Concatena o texto de várias chaves (string ou lista de strings)."""
    partes = []
    for chave in chaves:
        v = dados.get(chave, "")
...
def verificar(dados: dict) -> tuple[list[str], list[str]]:
    """
    Verifica consistência semântica do JSON.

    Retorna:
        erros   — contradições claras (o documento provavelmente está errado)
        avisos  — inconsistências prováveis (merecem revisão manual)
    """
    erros:  list[str] = []
    avisos: list[str] = []
