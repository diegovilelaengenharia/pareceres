...
from formatacao import set_spacing, set_font, no_borders, add_field
...
def build_header(doc):
    ...
    set_spacing(p_thin, line=20, before=30, after=0)
...
def add_page_number_footer(doc):
    """Adiciona rodapé com 'Página X de Y'."""
    section = doc.sections[0]
    ...
    set_spacing(fp, line=240, before=0, after=0)

    r1 = fp.add_run('Página ')
    set_font(r1, size=SZ_RODAPE)
    r1.font.color.rgb = COR_CINZA_LEVE

    r2 = fp.add_run()
    set_font(r2, size=SZ_RODAPE)
    r2.font.color.rgb = COR_CINZA_LEVE
    add_field(r2, ' PAGE ')

    r3 = fp.add_run(' de ')
    set_font(r3, size=SZ_RODAPE)
    r3.font.color.rgb = COR_CINZA_LEVE

    r4 = fp.add_run()
    set_font(r4, size=SZ_RODAPE)
